...
class Curl
{
    ...
    public function buildUrl($url, $data = array()) {
        return self::appendUrlParam($url, $data);
    }

    public static function appendUrlParam($url, $data = array()) {
    ...
    public function exec($url, $options, $debug = false)
    {
        ...
        /**
         * @desc   增加一个retry once 的机制，保障curl因网络不稳定带来的失败
         * @author yangzhen 
         * @mail   yangz03@mysoft.com.cn
         */
        if($this->_status === 0){
//            echo 'one status:',  $this->_status,PHP_EOL;
            $output = curl_exec($ch);
            $this->_status = curl_getinfo($ch, CURLINFO_HTTP_CODE);
        }
    ...
    public function post($url, $data, $params = array(), $debug = false)
    {
        ...
        if(is_array($data)) {
            foreach($data as $k => $v ){
                //对于上传文件的，形式为 ['key'=>'@filepath']格式
                if(is_string($v) && $v{0} == '@') {
                    $hasFile = true;
                }
            }
        }
        //by fangl，这里用http_build_query以支持post多维数组
        $options[CURLOPT_POSTFIELDS] = is_string($data)||$hasFile?$data:http_build_query($data);
    ...
    public function head($url,$params=array(),$debug=false) {
        $url = $this->buildUrl($url,$params);
        $options = $this->getOptions();
